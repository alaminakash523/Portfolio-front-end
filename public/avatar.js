...
function setupScene(gltf) {
    ...
    const camera = new THREE.PerspectiveCamera(
      42, container.clientWidth / container.clientHeight);
    ...
    if (waveClip) {
        ...
        const delay = 5; // 2 seconds delay
    
        waveAction.getMixer().addEventListener('finished', () => {
            setTimeout(() => {
                waveAction.reset(); // Reset the animation to the start
                waveAction.play(); // Play the animation again
            }, delay * 1000); // Convert delay to milliseconds
        });
        waveAction.play();
    } else {
        console.warn('Waving animation not found!');
    }
}